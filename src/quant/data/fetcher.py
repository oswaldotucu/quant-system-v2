...
from config.instruments import TICKER_CLASS, TIMEFRAMES, YF_SYMBOLS
...
def fetch_incremental(
    ...
    ticker_dir = Path(data_dir) / TICKER_CLASS[ticker]
    ticker_dir.mkdir(parents=True, exist_ok=True)
    path = ticker_dir / f"{ticker}_{tf}.csv"
    ...
    return FetchResult(
        ticker=ticker, timeframe=tf, new_bars=new_count, last_date=last_date, status="updated"
    )
