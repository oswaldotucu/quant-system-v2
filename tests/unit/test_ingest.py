...
def _staging_path(staging: Path, ticker: str, filename: str) -> Path:
    """Resolve staging subfolder path for a ticker."""
    return staging / TICKER_CLASS[ticker] / filename


def _data_path(data_dir: Path, ticker: str, filename: str) -> Path:
    """Resolve data/raw subfolder path for a ticker."""
    return data_dir / TICKER_CLASS[ticker] / filename
...
class TestIngestFile:
    ...
    def test_append_new_bars(self, tmp_path: Path) -> None:
        ...
        _write_csv(_data_path(data_dir, "MNQ", "MNQ_15m.csv"), old_bars)

        # Staging has old bars + 50 new ones
        all_bars = _make_bars("2024-01-02 18:00", periods=150)
        _write_csv(_staging_path(staging, "MNQ", "MNQ_15m.csv"), all_bars)
        ...
        # Verify total rows
        result = pd.read_csv(
            _data_path(data_dir, "MNQ", "MNQ_15m.csv"), index_col=0, parse_dates=True
        )
    ...
    def test_no_new_bars(self, tmp_path: Path) -> None:
        ...
        bars = _make_bars("2024-01-02 18:00", periods=100)
        _write_csv(_data_path(data_dir, "MNQ", "MNQ_15m.csv"), bars)
        _write_csv(_staging_path(staging, "MNQ", "MNQ_15m.csv"), bars)
    ...
    def test_deduplicates_overlapping_timestamps(self, tmp_path: Path) -> None:
        ...
        _write_csv(_staging_path(staging, "MNQ", "MNQ_15m.csv"), staging_bars)
    ...
    def test_mini_contract_subfolder(self, tmp_path: Path) -> None:
        """Mini contracts (NQ, ES, GC) use mini/ subfolder."""
        from quant.data.ingest import ingest_file
        ...
        bars = _make_bars("2024-01-02 18:00", periods=50)
        _write_csv(_staging_path(staging, "NQ", "NQ_5m.csv"), bars)

        report = ingest_file("NQ", "5m", staging, data_dir)
        assert report.new_bars == 50
        assert report.status == "updated"
        assert _data_path(data_dir, "NQ", "NQ_5m.csv").exists()
...
class TestGapDetection:
    ...
    def test_detects_weekday_gap(self, tmp_path: Path) -> None:
        ...
        _write_csv(_staging_path(staging, "MNQ", "MNQ_15m.csv"), new_bars)
...
class TestIngestAll:
    """Tests for ingest() — all 18 files."""

    def test_ingest_all_files(self, tmp_path: Path) -> None:
        """Ingest processes all 18 instrument/timeframe combos."""
        ...
        for ticker in TICKERS:
            for tf in TIMEFRAMES:
                ...
                _write_csv(_staging_path(staging, ticker, f"{ticker}_{tf}.csv"), bars)

        report = ingest(staging, data_dir)
        assert report.files_updated == 18
        assert report.total_new_bars == 18 * 50
